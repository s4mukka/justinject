...
//
// Additionally, the Level attribute is included within the converted entry
// to provide further context on the log level used.
//
...
func (h *Hook) convertEntry(e *logrus.Entry) log.Record {
	...
	record.AddAttributes(
		append(
			convertFields(e.Data),
			log.KeyValue{Key: "level", Value: convertValue(convertLogLevel(e.Level))},
		)...,
	)
